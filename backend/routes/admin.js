...
// Statistika za panel administratora
...
// Svi događaji (za administratora) – sa ukupnom zaradom (samo PUBLISHED)
...
// Dohvati sve zahtjeve na čekanju
...
// Statistika uloga za kružni dijagram
...
router.put("/role-requests/:id/approve", requireAdmin, async (req, res, next) => {
    try {
        ...
        // Provjeri da li zahtjev postoji i da je na čekanju
        const [requests] = await pool.query(
            "SELECT user_id FROM role_requests WHERE id = ? AND status = 'pending'",
            [id]
        );
        if (!requests.length) {
            return res.status(404).json({ error: "Zahtjev nije pronađen ili je već obrađen" });
        }
        ...
        res.json({ message: "Zahtjev je odobren. Korisnik je sada organizator." });
    } catch (err) {
        next(err);
    }
});
...
router.put("/role-requests/:id/reject", requireAdmin, async (req, res, next) => {
    try {
        ...
        res.json({ message: "Zahtjev je uspješno odbijen." });
    } catch (err) {
        next(err);
    }
});
