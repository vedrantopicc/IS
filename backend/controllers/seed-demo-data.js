...
export async function seedDemoData(req, res) {
    try {
        ...
        if (existingStudents.length > 0) {
            ...
            console.log("📚 %d postojećih studenata pronađeno – koristim ih za rezervacije", studentIds.length);
        } else {
            console.log("🆕 Nisu pronađeni studenti – kreiram 100 demo studenata...");
            const studentValues = [];
            for (let i = 1; i <= 100; i++) {
                studentValues.push([
                    `student${i}`,
            ...
            console.log("✅ Kreirano 100 demo studenata");
        }
        ...
        if (allEvents.length === 0) {
            return res.status(400).json({
                error: "Nisu pronađeni događaji u bazi podataka. Prvo kreirajte barem jedan događaj."
            });
        }

        console.log("📊 Pronađeno %d događaja – obrađujem za demo rezervacije...", allEvents.length);
        ...
        for (const event of allEvents) {
            ...
            console.log("🧹 Očišćene postojeće demo rezervacije za događaj %s", eventId);
            ...
            if (ticketTypes.length === 0) {
                console.warn("⚠️ Događaj %s nema tipove ulaznica – preskačem", eventId);
                continue;
            }

            // Pripremi mapu za praćenje preostalih mjesta
            const ticketTypeMap = {};
         ...
         GROUP BY ticket_type_id`,
            ...
            for (let i = 0; i < reservationCount; i++) {
                ...
                // Filtriraj tipove sa preostalim mjestima
           ...
           VALUES ?`,
                ...
                console.log("✅ Dodato %d novih demo rezervacija za događaj %s", reservations.length, eventId);
                totalNewReservations += reservations.length;
            }
        }

        res.status(200).json({
            message: "✅ Demo rezervacije uspješno osvježene za sve događaje",
            stats: {
                eventsProcessed: allEvents.length,
                totalNewReservations,
                studentsUsed: studentIds.length,
                note: "Sve stare 'DEMO-' rezervacije su uklonjene. Nove poštuju ograničenja mjesta."
            }
        });

    } catch (err) {
        console.error("❌ Neuspješno punjenje rezervacija:", err);
        res.status(500).json({ error: "Neuspješno kreiranje demo rezervacija", message: err.message });
    }
}