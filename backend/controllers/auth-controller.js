import { createUser } from "./users-controller.js";
import { pool } from "../db.js";
import bcrypt from "bcrypt";
import jwt from "jsonwebtoken";
import { v4 as uuidv4 } from "uuid";
import { sendPasswordResetEmail } from "../utils/emailService.js";
import { randomBytes } from "crypto";

export async function register(req, res, _next) {
    try {
        const { name, surname, email, username, password } = req.body;
        const user = await createUser({ name, surname, email, username, password, isOrganizer: false });
        res.status(201).json(user);
    } catch (e) {
        res.status(400).json({ error: e.message });
    }
}

const JWT_SECRET = process.env.JWT_SECRET;
const JWT_EXPIRES = process.env.JWT_EXPIRES;

export async function login(req, res, _next) {
    try {
        const { email, username, password } = req.body ?? {};
        if ((!email && !username) || !password) {
            return res.status(400).json({ error: "E-mail/korisničko ime i lozinka su obavezni" });
        }

        const identifier = email || username;
        const [rows] = await pool.query(
            "SELECT id, username, name, surname, email, password, role, is_organizer FROM `user` WHERE email = ? OR username = ? LIMIT 1",
            [identifier, identifier]
        );

        if (!rows.length) {
            return res.status(401).json({ error: "Neispravni podaci za prijavu" });
        }

        const user = rows[0];
        const isMatch = await bcrypt.compare(password, user.password);
        if (!isMatch) {
            return res.status(401).json({ error: "Neispravni podaci za prijavu" });
        }

        // --- NOVO: Bilježenje aktivnosti pri uspješnoj prijavi ---
        try {
            await pool.query(
                "INSERT INTO user_activity (user_id) VALUES (?)",
                [user.id]
            );
        } catch (activityError) {
            // Bilježimo grešku u konzolu, ali ne prekidamo proces prijave
            console.error("Greška pri upisu u user_activity:", activityError.message);
        }
        // -------------------------------------------------------

        const { password: _pw, ...safeUser } = user;

        const jti = uuidv4();

        const payload = {
            sub: String(user.id),
            jti,
            username: user.username,
            name: user.name,
            surname: user.surname,
            email: user.email,
            role: user.role,
            is_organizer: user.is_organizer,
        };

        const token = jwt.sign(payload, JWT_SECRET, { expiresIn: JWT_EXPIRES });

        await pool.query("DELETE FROM token WHERE user_id = ?", [user.id]);

        await pool.query(
            "INSERT INTO token (id, user_id, valid, expires_at) VALUES (?, ?, 1, DATE_ADD(UTC_TIMESTAMP(), INTERVAL 7 DAY))",
            [jti, user.id]
        );

        return res.json({ token, user: safeUser });
    } catch (e) {
        res.status(500).json({ error: e.message || "Neuspješna prijava" });
    }
}

export async function changePassword(req, res, next) {
    try {
        const userId = req.user?.id;
        const { currentPassword, newPassword } = req.body || {};

        if (!userId) return res.status(401).json({ error: "Niste autorizirani" });
        if (!currentPassword || !newPassword) {
            return res.status(400).json({ error: "Trenutna i nova lozinka su obavezne" });
        }
        if (String(newPassword).length < 6) {
            return res.status(400).json({ error: "Nova lozinka mora imati najmanje 6 znakova" });
        }

        const [rows] = await pool.query(
            "SELECT id, password FROM `user` WHERE id = ? LIMIT 1",
            [userId]
        );
        if (!rows.length) return res.status(404).json({ error: "Korisnik nije pronađen" });

        const user = rows[0];

        const ok = await bcrypt.compare(currentPassword, user.password);
        if (!ok) return res.status(401).json({ error: "Trenutna lozinka nije ispravna" });

        const hash = await bcrypt.hash(newPassword, 10);
        await pool.query("UPDATE `user` SET password = ? WHERE id = ?", [hash, userId]);

        try {
            await pool.query("UPDATE `token` SET valid = 0 WHERE user_id = ?", [userId]);
        } catch { }

        return res.json({ message: "Lozinka je uspješno promijenjena" });
    } catch (err) {
        next(err);
    }
}

export async function logout(req, res, next) {
    try {
        const jti = req.user?.jti;
        const userId = req.user?.id;

        if (jti && userId) {
            try {
                await pool.query("DELETE FROM `token` WHERE id = ? AND user_id = ?", [jti, userId]);
            } catch (e) { }
        }

        return res.status(204).end();
    } catch (err) {
        next(err);
    }
}

// === NOVE FUNKCIJE ZA ZABORAVLJENU LOZINKU ===

export async function forgotPassword(req, res) {
    try {
        const { email } = req.body;
        if (!email) {
            return res.status(400).json({ error: "E-mail je obavezan" });
        }

        const [rows] = await pool.query("SELECT id FROM `user` WHERE email = ?", [email]);
        if (rows.length === 0) {
            return res.json({ message: "Poslat vam je e-mail za ponovno postavljanje lozinke." });
        }

        const token = randomBytes(32).toString("hex");
        const expiresAt = new Date(Date.now() + 30 * 60 * 1000); // +30 minuta

        await pool.query(
            "INSERT INTO password_reset_tokens (user_id, token, expires_at) VALUES (?, ?, ?)",
            [rows[0].id, token, expiresAt]
        );

        await sendPasswordResetEmail(email, token);

        return res.json({ message: "Poslat vam je e-mail za ponovno postavljanje lozinke." });
    } catch (e) {
        console.error("Greška pri zaboravljenoj lozinci:", e);
        return res.status(500).json({ error: "Greška na serveru" });
    }
}

export async function resetPassword(req, res) {
    try {
        const { token, newPassword } = req.body;
        if (!token || !newPassword) {
            return res.status(400).json({ error: "Token i nova lozinka su obavezni" });
        }
        if (newPassword.length < 6) {
            return res.status(400).json({ error: "Lozinka mora imati najmanje 6 znakova" });
        }

        const [rows] = await pool.query(
            "SELECT user_id FROM password_reset_tokens WHERE token = ? AND expires_at > NOW()",
            [token]
        );

        if (rows.length === 0) {
            return res.status(400).json({ error: "Token je neispravan ili je istekao" });
        }

        const hash = await bcrypt.hash(newPassword, 10);
        await pool.query("UPDATE `user` SET password = ? WHERE id = ?", [hash, rows[0].user_id]);
        await pool.query("DELETE FROM password_reset_tokens WHERE token = ?", [token]);

        return res.json({ message: "Lozinka je uspješno promijenjena!" });
    } catch (e) {
        console.error("Greška pri ponovnom postavljanju lozinke:", e);
        return res.status(500).json({ error: "Greška na serveru" });
    }
}