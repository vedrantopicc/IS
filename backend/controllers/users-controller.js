...
export async function createUser({ name, surname, email, username, password, isOrganizer = false }) {
    ...
    if (dupe.length) throw new Error("E-mail ili korisničko ime je već u upotrebi");
...
export async function updateUserById(req, res, next) {
    try {
        ...
        if (!name && !surname && !role) {
            return res.status(400).json({ error: "Nema važećih polja za ažuriranje" });
        }
        ...
        if (role && req.user.role !== "Admin") {
            return res.status(403).json({ error: "Samo administratori mogu mijenjati uloge korisnika" });
        }
...
export async function deleteUserById(req, res, next) {
    try {
        ...
        if (!existingUser.length) return res.status(404).json({ error: "Korisnik nije pronađen" });
        if (existingUser[0].role === "Admin") return res.status(403).json({ error: "Nije moguće obrisati administratora" });

        await pool.query("UPDATE `user` SET deleted_at = NOW() WHERE id = ?", [id]);
        res.json({ message: "Korisnik je uspješno obrisan", deletedUser: existingUser[0] });
    } catch (err) {
        next(err);
    }
}
...
export async function restoreUserById(req, res, next) {
    try {
        ...
        if (result.affectedRows === 0) return res.status(404).json({ error: "Korisnik nije pronađen ili nije obrisan" });
        res.json({ message: "Korisnik je uspješno vraćen" });
    } catch (err) {
        next(err);
    }
}
...
// 8. STATISTIKA ULOGA (Raspodjela uloga)
    ...
    try {
        ...
        switch (period) {
            ...
            case 'yearly':
                ...
                break;
            default:
                return res.status(400).json({ message: "Nevažeći period" });
        }
