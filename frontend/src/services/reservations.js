...
export async function createReservation(eventId, ticketTypeId, numberOfTickets = 1) {
    ...
    if (!res.ok) throw new Error(data.error || "Neuspješno kreiranje rezervacije");
    return data;
}
...
export async function getUserReservations() {
    ...
    const data = await res.json().catch(() => ({}));
    if (!res.ok) throw new Error(data.error || "Neuspješno preuzimanje rezervacija");
    return data;
}
...
export async function deleteReservation(reservationId) {
    ...
    const data = await res.json().catch(() => ({}));
    if (!res.ok) throw new Error(data.error || "Neuspješno brisanje rezervacije");
    return data;
}
...
export async function getReservationById(reservationId) {
    ...
    const data = await res.json().catch(() => ({}));
    if (!res.ok) throw new Error(data.error || "Neuspješno preuzimanje rezervacije");
    return data;
}