...
function formatDate(dateString) {
    ...
    if (diffInSeconds < 3600) return `prije ${Math.floor(diffInSeconds / 60)} minuta`;
    if (diffInSeconds < 86400) return `prije ${Math.floor(diffInSeconds / 3600)} sati`;
    if (diffInSeconds < 2592000) return `prije ${Math.floor(diffInSeconds / 86400)} dana`;
...
export default function Comments({ eventId, isOrganizer = false }) {
    ...
    const loadComments = async () => {
        try {
            ...
            toast.error("Učitavanje recenzija nije uspjelo");
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleAddComment = async () => {
        if (newRating < 1 || newRating > 5) {
            toast.error("Molimo odaberite ocjenu (1-5 zvjezdica)");
            return;
        }

        if (newComment.length > 1000) {
            toast.error("Komentar je predugačak (maks. 1000 znakova)");
            return;
        }
        ...
        try {
            ...
            toast.error(error.message || "Dodavanje recenzije nije uspjelo");
        } finally {
            setSubmitting(false);
        }
    };
    ...
    const handleEditComment = async (commentId) => {
        ...
        try {
            ...
            toast.error(error.message || "Ažuriranje recenzije nije uspjelo");
        } finally {
            setSubmitting(false);
        }
    };
    ...
    const handleDeleteComment = async (commentId) => {
        ...
        try {
            ...
            toast.error(error.message || "Brisanje recenzije nije uspjelo");
        } finally {
            setSubmitting(false);
        }
    };
            ...
            {isStudent && !userReview && !localIsOrganizer && !isOrganizer ? (
                <div className="space-y-3 border-b border-gray-200 pb-4">
                    ...
                    <Textarea
                        ...
                        placeholder="Podijelite svoje iskustvo o ovom događaju..."
                    ...
                    <div className="flex items-center justify-between">
                        <span className="text-sm text-gray-500">
                            {newComment.length}/1000 znakova
                        </span>
                        <Button
                        ...
                        >
                            {submitting ? "Dodavanje..." : "Pošalji recenziju"}
                        </Button>
                    </div>
                </div>
            ) : null}
                                            ...
                                            <div className="flex items-center gap-2 flex-wrap">
                                                ...
                                                <span className="text-sm text-gray-500">
                                                    ...
                                                    {comment.updated_at !== comment.created_at && (
                                                        <span className="ml-1">(izmijenjeno)</span>
                                                    )}
                                                </span>
                                            </div>
                                        </div>
