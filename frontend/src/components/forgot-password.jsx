...
export default function ForgotPassword() {
    ...
    const handleSubmit = async (e) => {
        ...
        if (!validateEmail(email)) {
            setErrors({ email: "Molimo unesite ispravan email" });
            return;
        }
        ...
        try {
            ...
            if (res.ok) {
                toast.success("Ako nalog postoji, dobićete email za resetovanje lozinke.");
                setTimeout(() => navigate("/"), 3000);
            } else {
                toast.error(data.error || "Slanje emaila za resetovanje nije uspjelo.");
            }
        } catch (err) {
