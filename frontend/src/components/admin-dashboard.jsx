...
async function getUserRoleStats() {
    ...
    if (!res.ok) throw new Error("Greška pri preuzimanju statistike uloga");
    return res.json();
}
...
async function getRoleRequests() {
    ...
    if (!res.ok) throw new Error("Greška pri preuzimanju zahtjeva za uloge");
    return res.json();
}
...
async function approveRoleRequest(requestId) {
    ...
    if (!res.ok) throw new Error("Greška pri odobravanju zahtjeva");
    return res.json();
}
...
async function rejectRoleRequest(requestId) {
    ...
    if (!res.ok) throw new Error("Greška pri odbijanju zahtjeva");
    return res.json();
}
...
function getDisplayName(p) {
    if (!p) return "Administrator";
    if (p.name && p.surname) return `${p.name} ${p.surname}`;
    return p.name || p.username || p.email || "Administrator";
}
...
// ✅ FUNKCIJA: vraća ulogu na osnovu is_organizer polja sa fallback logikom
...
export default function AdminDashboard() {
    ...
    const loadDashboardData = async () => {
        ...
        try {
            ...
            toast.error("Greška pri učitavanju administratorske kontrolne table: " + err.message);
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleApproveRequest = async (requestId, username) => {
        try {
            ...
            toast.success(`Zahtjev odobren za ${username}`);
            loadDashboardData();
        } catch (err) {
            toast.error("Greška pri odobravanju zahtjeva: " + err.message);
        } finally {
    ...
    const handleRejectRequest = async (requestId, username) => {
        try {
            ...
            toast.success(`Zahtjev odbijen za ${username}`);
            const requests = await getRoleRequests();
            setRoleRequests(requests);
        } catch (err) {
            toast.error("Greška pri odbijanju zahtjeva: " + err.message);
        } finally {
            setLoadingRequests(false);
        }
    };
    ...
    const handleDeleteUser = async (userId, username) => {
        try {
            ...
            toast.success(`Korisnik ${username} uspješno obrisan`);
            loadDashboardData();
        } catch (err) {
            toast.error("Greška pri brisanju korisnika: " + err.message);
        }
    };
    ...
    const handleRestoreUser = async (userId, username) => {
        try {
            ...
            toast.success(`Korisnik ${username} uspješno vraćen`);
            loadDashboardData();
        } catch (err) {
            toast.error("Greška pri vraćanju korisnika: " + err.message);
        }
    };
    ...
    const handleDeleteEvent = async (eventId, title) => {
        try {
            ...
            toast.success(`Događaj "${title}" uspješno obrisan`);
        ...
        return (
            <div className="min-h-screen bg-gray-50 flex items-center justify-center">
                <div className="text-center">
                    ...
                    <p className="mt-2 text-gray-600">Učitavanje administratorske kontrolne table...</p>
                </div>
            </div>
        );
    }
    ...
    const userCount = users.filter(user => user.displayRole !== 'Administrator').length;
                        ...
                        <DropdownMenuContent align="end" className="w-60 text-gray-900 bg-white shadow-xl border border-gray-200 rounded-md z-[9999] mt-2">
                            ...
                            <div className="p-1">
                                <DropdownMenuItem onClick={() => navigate("/events")} className="flex items-center px-3 py-2 text-sm hover:bg-gray-100 cursor-pointer rounded-sm">
                                    <Calendar className="mr-3 h-4 w-4" /> <span>Pregled događaja</span>
                                </DropdownMenuItem>
                                <DropdownMenuItem onClick={() => navigate("/settings")} className="flex items-center px-3 py-2 text-sm hover:bg-gray-100 cursor-pointer rounded-sm">
                                    <Settings className="mr-3 h-4 w-4" /> <span>Postavke</span>
                                </DropdownMenuItem>
                            </div>
                            ...
                            <div className="p-1">
                                <DropdownMenuItem onClick={handleLogout} className="flex items-center px-3 py-2 text-sm hover:bg-red-50 hover:text-red-700 cursor-pointer rounded-sm text-red-600">
                                    <LogOut className="mr-3 h-4 w-4" /> <span>Odjava</span>
                                </DropdownMenuItem>
                            </div>
                        </DropdownMenuContent>
                    </DropdownMenu>
            ...
            <div className="mx-auto max-w-7xl px-4 py-6">
                ...
                <Tabs value={activeTab} onValueChange={handleTabChange}>
                    <TabsList className="flex w-full justify-start gap-8 bg-transparent rounded-none p-0 h-12 overflow-x-auto overflow-y-hidden">
                        <TabsTrigger value="dashboard" className="flex items-center gap-2 text-black"><BarChart3 className="w-4 h-4" /> Kontrolna tabla</TabsTrigger>
                        <TabsTrigger value="users" className="flex items-center gap-2 text-black"><Users className="w-4 h-4" /> Korisnici ({userCount})</TabsTrigger>
                        <TabsTrigger value="deleted" className="flex items-center gap-2 text-black"><Trash2 className="w-4 h-4" /> Obrisani ({deletedUsers.length})</TabsTrigger>
                        <TabsTrigger value="role-requests" className="flex items-center gap-2 text-black"><UserCheck className="w-4 h-4" /> Zahtjevi ({roleRequests.length})</TabsTrigger>
                        <TabsTrigger value="events" className="flex items-center gap-2 text-black"><Calendar className="w-4 h-4" /> Događaji ({events.length})</TabsTrigger>
                        <TabsTrigger value="statistics" className="flex items-center gap-2 text-black"><TrendingUp className="w-4 h-4" /> Statistika</TabsTrigger>
                    </TabsList>
                        ...
                        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
                            <Card>
                                <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                                    <CardTitle className="text-sm font-medium text-black">Ukupno korisnika</CardTitle>
                                ...
                                <CardContent>
                                    ...
                                    <p className="text-xs text-gray-500">
                                        {roleStats.students} studenata, {roleStats.organizers} organizatora
                                    </p>
                                </CardContent>
                            </Card>
                            ...
                            <Card>
                                <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                                    <CardTitle className="text-sm font-medium text-black">Ukupno događaja</CardTitle>
                                    <Calendar className="h-4 w-4 text-gray-500" />
                                </CardHeader>
                                <CardContent>
                                    <div className="text-2xl font-bold text-black">{dashboardData?.events?.total_events || 0}</div>
                                    <p className="text-xs text-gray-500">{dashboardData?.events?.upcoming_events || 0} predstojećih</p>
                                </CardContent>
                            </Card>

                            <Card>
                                <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                                    <CardTitle className="text-sm font-medium text-black">Prošli događaji</CardTitle>
                            ...
                            <Card>
                                <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                                    <CardTitle className="text-sm font-medium text-black">Administratori</CardTitle>
                    ...
                    <TabsContent value="users" className="mt-6">
                        <Card>
                            <CardHeader>
                                <CardTitle className="text-gray-900">Upravljanje korisnicima</CardTitle>
                            ...
                            <CardContent>
                                ...
                                <Table>
                                    <TableHeader>
                                        <TableRow>
                                            <TableHead className="text-black font-bold">Korisnik</TableHead>
                                            <TableHead className="text-black font-bold">E-pošta</TableHead>
                                            <TableHead className="text-black font-bold">Uloga</TableHead>
                                            <TableHead className="text-black font-bold text-right">Radnje</TableHead>
                                        </TableRow>
                                    </TableHeader>
                                    <TableBody>
                                        {users
                                            .filter(user => {
                                                ...
                                                return user.displayRole !== 'Administrator' && (user.name.toLowerCase().includes(s) || user.email.toLowerCase().includes(s) || user.username.toLowerCase().includes(s));
                                            })
                                            .map((user) => (
                                                <TableRow key={user.id}>
                                                    ...
                                                    <TableCell>
                                                        {/* ✅ Prikaz uloge - uklonjen debug info, prilagođen font */}
                                                        <Badge
                                                            variant={user.displayRole === 'Organizator' ? 'default' : 'secondary'}
                                                    ...
                                                    <TableCell className="text-right">
                                                        <div className="flex justify-end gap-2">
                                                            ...
                                                            <AlertDialog>
                                                                ...
                                                                <AlertDialogContent>
                                                                    <AlertDialogHeader><AlertDialogTitle>Obriši korisnika</AlertDialogTitle><AlertDialogDescription>Obriši "{user.username}"?</AlertDialogDescription></AlertDialogHeader>
                                                                    <AlertDialogFooter>
                                                                        <AlertDialogCancel>Otkaži</AlertDialogCancel>
                                                                        <AlertDialogAction onClick={() => handleDeleteUser(user.id, user.username)} className="bg-red-600 hover:bg-red-700">Obriši</AlertDialogAction>
                                                                    </AlertDialogFooter>
                                                                </AlertDialogContent>
                                                            </AlertDialog>
                                                        </div>
                    ...
                    <TabsContent value="deleted" className="mt-6">
                        <Card>
                            <CardHeader><CardTitle className="text-gray-900">Obrisani korisnici</CardTitle></CardHeader>
                            <CardContent>
                                <Table>
                                    <TableHeader>
                                        <TableRow>
                                            <TableHead className="text-black">Korisnik</TableHead>
                                            <TableHead className="text-black">Obrisano dana</TableHead>
                                            <TableHead className="text-right text-black">Radnje</TableHead>
                                        </TableRow>
                                    </TableHeader>
                                    <TableBody>
                                        {deletedUsers.length > 0 ? (
                                            ...
                                            <TableRow><TableCell colSpan={3} className="text-center py-4 text-gray-500">Nema obrisanih korisnika.</TableCell></TableRow>
                                        )}
                                    </TableBody>
                                </Table>
                            </CardContent>
                    ...
                    <TabsContent value="role-requests" className="mt-6">
                        <Card>
                            <CardHeader><CardTitle className="text-gray-900">Zahtjevi za ulogom</CardTitle></CardHeader>
                            <CardContent>
                                <Table>
                                    <TableHeader>
                                        <TableRow>
                                            <TableHead className="text-black">Korisnik</TableHead>
                                            <TableHead className="text-black">Podneseno</TableHead>
                                            <TableHead className="text-right text-black">Radnje</TableHead>
                                        </TableRow>
                                    </TableHeader>
                                    <TableBody>
                                        {roleRequests.length > 0 ? (
                                            ...
                                            <TableRow>
                                                <TableCell colSpan={3} className="text-center py-4 text-gray-500">Nema zahtjeva na čekanju</TableCell>
                                            </TableRow>
                                        )}
                                    </TableBody>
                                </Table>
                    ...
                    <TabsContent value="events" className="mt-6">
                        <Card>
                            <CardHeader><CardTitle className="text-gray-900">Upravljanje događajima</CardTitle></CardHeader>
                            <CardContent>
                                <Table>
                                    <TableHeader>
                                        <TableRow>
                                            <TableHead className="text-black font-bold w-1/4">Događaj</TableHead>
                                            <TableHead className="text-black font-bold w-1/6">Organizator</TableHead>
                                            <TableHead className="text-black font-bold w-1/6">Datum</TableHead>
                                            <TableHead className="text-black font-bold w-1/6">Prihod</TableHead>
                                            <TableHead className="text-black font-bold text-right w-1/6">Radnje</TableHead>
                                        </TableRow>
                                    </TableHeader>
                                    <TableBody>
                                        {events.map((event) => (
                    ...
                    <TabsContent value="statistics" className="mt-6">
                        <div className="grid gap-6 md:grid-cols-1 lg:grid-cols-3">
                            <Card className="lg:col-span-2">
                                <CardHeader>
                                    <div className="flex items-center gap-2">
                                        ...
                                        <CardTitle className="text-black">Pregled aktivnosti korisnika</CardTitle>
                                    </div>
                                </CardHeader>
                            ...
                            <div className="space-y-6">
                                <Card>
                                    <CardHeader>
                                        <div className="flex items-center gap-2">
                                            ...
                                            <CardTitle className="text-black">Udio uloga</CardTitle>
                                        </div>
                                    </CardHeader>
                                    <CardContent className="py-6">
                                        <div className="w-full space-y-4">
                                            ...
                                            <div className="flex justify-between items-center">
                                                <span className="text-sm text-gray-600">Organizatori</span>
                                ...
                                <Card>
                                    <CardHeader><CardTitle className="text-black">Uvidi u prihode</CardTitle></CardHeader>
                                    <CardContent>
                                        <div className="text-center">
            ...
            {selectedUser && (
                <AlertDialog open={showUserDetails} onOpenChange={setShowUserDetails}>
                    <AlertDialogContent className="bg-white max-w-md">
                        <AlertDialogHeader><AlertDialogTitle className="text-black">Detalji korisnika</AlertDialogTitle></AlertDialogHeader>
                        <div className="space-y-3 py-4 text-sm text-black">
                            <p><strong>Ime:</strong> {selectedUser.name} {selectedUser.surname}</p>
                            <p><strong>Korisničko ime:</strong> @{selectedUser.username}</p>
                            <p><strong>E-pošta:</strong> {selectedUser.email}</p>
                            {/* ✅ Modal sada prikazuje displayRole */}
                            <p><strong>Uloga:</strong> {selectedUser.displayRole || selectedUser.role}</p>
                            {selectedUser.bio && <p><strong>Biografija:</strong> {selectedUser.bio}</p>}
                        </div>
                        <AlertDialogFooter>
                            <AlertDialogCancel onClick={() => setShowUserDetails(false)} className="cursor-pointer">Zatvori</AlertDialogCancel>
                        </AlertDialogFooter>
                    </AlertDialogContent>
                </AlertDialog>
            )}
